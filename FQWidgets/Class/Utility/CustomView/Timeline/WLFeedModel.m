...
- (BOOL)modelCustomTransformFromDictionary:(NSDictionary *)dic {
    WLPictureMetadata *meta = _large ? _large : _largest ? _largest : _original;
    _badgeType = meta.badgeType;
    return YES;
}
@end

@implementation WLURL
+ (NSDictionary *)modelCustomPropertyMapper {
...
@implementation WLButtonLink
@end

@implementation WLPageInfo
+ (NSDictionary *)modelCustomPropertyMapper {
...
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{@"buttons" : [WLButtonLink class]};
}
@end

@implementation WLFeedModelTitle
+ (NSDictionary *)modelCustomPropertyMapper {
...
- (BOOL)modelCustomTransformFromDictionary:(NSDictionary *)dic {
    ...
    // 这个不一定准。。
    if (_verified) {
        _userVerifyType = WLUserVerifyTypeStandard;
    } else if (_verifiedType == 220) {
        _userVerifyType = WLUserVerifyTypeClub;
    } else if (_verifiedType == -1 && _verifiedLevel == 3) {
        _userVerifyType = WLUserVerifyTypeOrganization;
    } else {
        _userVerifyType = WLUserVerifyTypeNone;
    }
    return YES;
}
@end
...
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{@"picIds" : [NSString class],
             @"picInfos" : [WLPicture class],
             @"urlStruct" : [WLURL class],
             @"topicStruct" : [WLTopic class],
             @"tagStruct" : [WLTag class]};
}
- (BOOL)modelCustomTransformFromDictionary:(NSDictionary *)dic {
    // 自动 model-mapper 不能完成的，这里可以进行额外处理
    _pics = nil;
...
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{@"statuses" : [WLFeedModel class]};
}
@end
