...
@interface FQCutImageViewController () <UIScrollViewDelegate>
...
@implementation FQCutImageViewController
...
- (UIView *)operateView {
    if (!_operateView) {
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds) - kNavBarHeight - kSizeScale(60) - kSafeAreaBottomY, kScreenWidth, kSizeScale(60))];
        _operateView = view;
        
        CGFloat paddingX = kSizeScale(15);
        
        _cancelBtn = [self buttonWithTitle:@"CANCEL" action:@selector(cancelBtnClicked)];
        [view addSubview:_cancelBtn];
        [_cancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(view).offset(paddingX);
            make.centerY.mas_equalTo(view);
            make.size.mas_equalTo(CGSizeMake(kSizeScale(100), kSizeScale(30)));
        }];
        
        _confirmBtn = [self buttonWithTitle:@"DONE" action:@selector(confirmBtnClicked)];
        [view addSubview:_confirmBtn];
        [_confirmBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.mas_equalTo(view).offset(-paddingX);
            make.centerY.mas_equalTo(_cancelBtn);
            make.size.mas_equalTo(_cancelBtn);
        }];
    }
    return _operateView;
}
...
- (UIButton *)buttonWithTitle:(NSString *)title action:(SEL)action {
    ...
    btn.titleLabel.font = kMediumFont(kSizeScale(15));
