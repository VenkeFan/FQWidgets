...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
       // self.backgroundColor = [AliyunIConfig config].timelineBackgroundCollor;
        ...
        //bgView.image = [AppContext getImageForKey:@"ProgressBg"];
...
- (void)defaultParam {
    ...
    _colorProgress = [UIColor redColor];//[AliyunIConfig config].timelineTintColor;
    _colorSepatorPoint = [UIColor whiteColor];
    _colorSelect = [UIColor orangeColor];//[AliyunIConfig config].timelineDeleteColor;
    _selectedIndex = -1;
}
