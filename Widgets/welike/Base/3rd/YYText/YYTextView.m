...
//guoyibo add
-(void)setExtraBottomViewSize:(CGFloat)extraBottomViewSize
{
    _extraBottomViewSize = extraBottomViewSize;
    
    
    if (kScreenHeight == 480)
    {
       _extraBottomViewSize = extraBottomViewSize + 70;//iphone 4需要单独处理下
    }
    else
    {
        _extraBottomViewSize = extraBottomViewSize;
...
- (void)_updateLayout {
    ...
    //self.contentSize = size; //original code
    self.contentSize = CGSizeMake(size.width, size.height + _extraBottomViewSize);//guoyibo add
}
...
//            if (notify) [_inputDelegate selectionDidChange:self];
    ...
    [_innerText replaceCharactersInRange:range.asRange withString:text];//这一步
...
- (BOOL)_parseText {
    if (self.textParser) {
        ...
        BOOL textChanged = [self.textParser parseText:_innerText selectedRange:&newRange withRange:NSMakeRange(newRange.location, 0)];
...
- (BOOL)_parseText:(NSString *)key {
    if (self.textParser) {
        YYTextRange *oldTextRange = _selectedTextRange;
        NSRange newRange = _selectedTextRange.asRange;
        
        [_inputDelegate textWillChange:self];
        BOOL textChanged = [self.textParser parseText:_innerText selectedRange:&newRange withRange:NSMakeRange(newRange.location, key.length)];
        [_inputDelegate textDidChange:self];
...
{
    //NO  not scroll YES can scroll,when add uicontrol to UIscrollView
    return YES;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([self.delegate respondsToSelector:@selector(tapTextView)]) {
