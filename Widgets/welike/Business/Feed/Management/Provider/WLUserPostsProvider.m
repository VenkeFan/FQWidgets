...
@interface WLUserPostsProvider ()

@property (nonatomic, copy) NSString *uid;
@property (nonatomic, strong) WLUserPostsRequest *feedsRequest;
@property (nonatomic, copy) NSString *cursor;
@property (nonatomic, weak) id<WLFeedsProviderDelegate> delegate;

@end

@implementation WLUserPostsProvider
...
- (void)tryRefreshFeeds
{
    ...
    self.feedsRequest = [[WLUserPostsRequest alloc] initUserPostsRequestWithUid:self.uid];
    [self.feedsRequest listWithCursor:nil successed:^(NSArray *feeds, NSString *cursor) {
...
- (void)tryHisFeeds
{
    ...
    if ([self.cursor length] != 0)
    {
        __weak typeof(self) weakSelf = self;
        self.feedsRequest = [[WLUserPostsRequest alloc] initUserPostsRequestWithUid:self.uid];
        [self.feedsRequest listWithCursor:self.cursor successed:^(NSArray *feeds, NSString *cursor) {
            weakSelf.feedsRequest = nil;
            weakSelf.cursor = cursor;
            BOOL last = [weakSelf.cursor length] == 0;
