...
@interface WLTopicLatestProvider ()

@property (nonatomic, copy) NSString *topicID;
@property (nonatomic, strong) WLTopicLatestRequest *feedsRequest;
@property (nonatomic, copy) NSString *cursor;
@property (nonatomic, weak) id<WLFeedsProviderDelegate> delegate;

@end

@implementation WLTopicLatestProvider
...
- (void)tryRefreshFeeds {
    ...
    self.feedsRequest = [[WLTopicLatestRequest alloc] initWithTopicID:self.topicID];
    [self.feedsRequest tryTopicLatestWithCursor:nil successed:^(NSArray *feeds, NSString *cursor) {
...
- (void)tryHisFeeds {
    ...
    if ([self.cursor length] != 0)
    {
        __weak typeof(self) weakSelf = self;
        self.feedsRequest = [[WLTopicLatestRequest alloc] initWithTopicID:self.topicID];
        [self.feedsRequest tryTopicLatestWithCursor:self.cursor successed:^(NSArray *feeds, NSString *cursor) {
            weakSelf.feedsRequest = nil;
            self.cursor = cursor;
            BOOL last = [weakSelf.cursor length] == 0;
