...
@interface WLLatestFeedsProvider ()

@property (nonatomic, strong) WLLatestFeedsRequest *feedsRequest;
@property (nonatomic, copy) NSString *cursor;
@property (nonatomic, weak) id<WLFeedsProviderDelegate> delegate;

@end

@implementation WLLatestFeedsProvider
...
- (void)tryRefreshFeeds
{
    ...
    self.feedsRequest = [[WLLatestFeedsRequest alloc] initLatestFeedsRequest];
    [self.feedsRequest tryLatestFeedsWithCursor:nil successed:^(NSArray *feeds, NSString *cursor) {
...
- (void)tryHisFeeds
{
    ...
    if ([self.cursor length] != 0)
    {
        __weak typeof(self) weakSelf = self;
        self.feedsRequest = [[WLLatestFeedsRequest alloc] initLatestFeedsRequest];
        [self.feedsRequest tryLatestFeedsWithCursor:self.cursor successed:^(NSArray *feeds, NSString *cursor) {
            weakSelf.feedsRequest = nil;
