...
@interface WLCreatedCommentsProvider ()
...
@implementation WLCreatedCommentsProvider
...
- (void)tryRefreshCommentsForPid:(NSString *)pid
{
    ...
    [self.commentsRequest listCommentsWithSort:WELIKE_COMMENTS_SORT_CREATED cursor:nil successed:^(NSArray *comments, NSString *cursor) {
...
- (void)tryHisCommentsForPid:(NSString *)pid
{
    ...
    if ([self.cursor length] != 0)
    {
        __weak typeof(self) weakSelf = self;
        self.commentsRequest = [[WLCommentsRequest alloc] initCommentsRequestWithPid:pid];
        [self.commentsRequest listCommentsWithSort:WELIKE_COMMENTS_SORT_CREATED cursor:self.cursor successed:^(NSArray *comments, NSString *cursor) {
            weakSelf.commentsRequest = nil;
