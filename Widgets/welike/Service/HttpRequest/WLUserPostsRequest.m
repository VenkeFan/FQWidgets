...
#import "WLUserPostsRequest.h"
#import "WLPostBase.h"
#import "NSDictionary+JSON.h"
#import "WLAccountManager.h"

@implementation WLUserPostsRequest

- (id)initUserPostsRequestWithUid:(NSString *)uid
{
    if ([[[AppContext getInstance] accountManager] isLogin])
    {
        return [super initWithType:AFHttpOperationTypeNormal api:[NSString stringWithFormat:@"feed/user/%@/posts", uid] method:AFHttpOperationMethodGET];
    }
    else
    {
        
        return [super initWithType:AFHttpOperationTypeNormal api:[NSString stringWithFormat:@"feed/user/h5/%@/posts", uid] method:AFHttpOperationMethodGET];
    }
}
...
- (void)listWithCursor:(NSString *)cursor successed:(userPostsSuccessed)successed error:(failedBlock)error
{
   ...
   // [self.params setObject:[NSNumber numberWithInteger:1] forKey:@"count"];
    if ([cursor length] > 0)
    {
        ...
        if ([result isKindOfClass:[NSDictionary class]] == YES)
        {
            ...
            if ([list count] > 0)
            {
                ...
                for (NSInteger i = 0; i < [list count]; i++)
                {
                    ...
                    post.trackerSource = WLTrackerFeedSource_User_Posts;
                    if (post != nil)
                    {
            ...
            {
                [WLTrackerFeed setEmptyDataTrackerSource:WLTrackerFeedSource_User_Posts];
