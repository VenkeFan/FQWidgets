...
@implementation WLVerticalRequest

- (instancetype)init
{
    if ([AppContext getInstance].accountManager.isLogin) {
        return [super initWithType:AFHttpOperationTypeNormal api:[NSString stringWithFormat:@"user/interest/vertical/list"] method:AFHttpOperationMethodGET];
    } else {
        return [super initWithType:AFHttpOperationTypeNormal api:[NSString stringWithFormat:@"user/interest/skip/vertical/list"] method:AFHttpOperationMethodGET];
    }
}
...
- (void)requestVerticalFeedsWithPageNum:(NSInteger)pageNum successed:(VerticalRequestSuccessed)successed error:(failedBlock)error
{
    ...
    [self.params setObject:[NSNumber numberWithInteger:VerticalTypeCount] forKey:@"pageSize"];
