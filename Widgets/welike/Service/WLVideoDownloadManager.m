...
- (void)convertToMP4:(NSString *)fileUrlStr saved:(BOOL)saved finished:(void (^)(BOOL, NSURL *, NSError *))finished {
    ...
    NSString *exportPath = [NSString stringWithFormat:@"%@%@", NSTemporaryDirectory(), @"movie_fq_export.mp4"];
    KMMediaAsset *tsAsset = [KMMediaAsset assetWithURL:[NSURL fileURLWithPath:fileUrlStr] withFormat:KMMediaFormatTS];
    NSURL *exportPathUrl = [NSURL fileURLWithPath:exportPath];
    KMMediaAsset *mp4Asset = [KMMediaAsset assetWithURL:exportPathUrl withFormat:KMMediaFormatMP4];
    KMMediaAssetExportSession *exportSession = [[KMMediaAssetExportSession alloc] initWithInputAssets:@[tsAsset]];
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^{
        ...
        if (exportSession.status == KMMediaAssetExportSessionStatusCompleted) {
            if (saved) {
                [self saveVideoToPhotoLibrary:exportPathUrl
                                     finished:^(NSError *error) {
                                         ...
                                         [[NSFileManager defaultManager] removeItemAtPath:exportPathUrl.absoluteString error:nil];
                                     }];
            }
        }
        
        if (finished) {
            finished(exportSession.status == KMMediaAssetExportSessionStatusCompleted, exportPathUrl, exportSession.error);
        }
    }];
